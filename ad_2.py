...
def preprocessing(inputs):
  ...
  for i in range(0,n,16):
    input.append(inputs[i:(i+16)])
  return input

def corruption(input):
  counter = 0
  n = len(input)
  for i in range(0, n):
    counter += (max(input[i]) - min(input[i]))
  return counter

input = preprocessing(inputs)
counter = corruption(input)
